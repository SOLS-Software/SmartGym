          ...
          {drawerMode === 'student' ? (
            <form className="drawer-fields" onSubmit={handleSaveStudent}>
              ...
              <RegistrationField error={studentErrors.name} htmlFor="nmAluno" label="Nome" required size="full" touched={touchedStudentFields.name}>
                <input className={touchedStudentFields.name && studentErrors.name ? 'invalid' : ''} id="nmAluno" maxLength={255} onBlur={() => validateStudentField('name')} onChange={(event) => { const value = event.target.value; setStudentName(value); if (touchedStudentFields.name) { setStudentErrors((current) => ({ ...current, name: getStudentNameError(value) })); } }} placeholder="Ex.: Maria Souza" ref={nameInputRef} type="text" value={studentName} />
              </RegistrationField>
              {/* CPF */}
